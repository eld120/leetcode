...
def test_ll_builder():
    assert build_ll([1, 2, 1, 3, 5]) == ListNode(
        1, ListNode(2, ListNode(1, ListNode(3, ListNode(5))))
    )
...
if __name__ == "__main__":
    for num in reversed(range(0, 100)):
        print(
            f"{num} -- {has_cycle(build_ll_cycle([x for x in range(0,100)], num))} iterations to find the loop"
        )
